...
import h5py
import math
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, savePatchAsImage, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch, to_percentiles
import itertools
...
def safe_read_region(slide, coordinates, level, sizes, slide_path):
    try:
        region = slide.read_region(coordinates, level, sizes)
        return region
    except openslide.lowlevel.OpenSlideError:
        print(f"Skipped corrupt patch at {coordinates}, replacing with downsampled level 0")
        ...
        try:
            ...
            ds_sizes = tuple(dim * ds_factor for dim in sizes)
            level_0_region = slide.read_region(coordinates, 0, ds_sizes)
            downsampled_region = level_0_region.resize(sizes, Image.Resampling.LANCZOS)
            return downsampled_region
        except Exception as e:
            print(f"Error downsampling level 0 region at {coordinates}: {e}")
            # Return None or handle differently as per your need
            return None
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def safe_read_region(self, coordinates, level, sizes):
        try:
            region = self.wsi.read_region(coordinates, level, sizes)
            return region
        except openslide.lowlevel.OpenSlideError:
            print(f"Skipped corrupt patch at {coordinates}, replacing with downsampled level 0")
            ...
            try:
                self.wsi.close()
                self.wsi = openslide.OpenSlide(str(self.path))
                ds_factor = int(self.wsi.level_downsamples[level])
                ds_sizes = tuple(dim * ds_factor for dim in sizes)
                level_0_region = self.wsi.read_region(coordinates, 0, ds_sizes)
                downsampled_region = level_0_region.resize(sizes, Image.Resampling.LANCZOS)
                return downsampled_region
            except Exception as e:
                print(f"Error downsampling level 0 region at {coordinates}: {e}")
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False,
        ...
        img = np.array(self.safe_read_region((0,0), seg_level, self.level_dim[seg_level]))
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0),
        ...
        img = np.array(self.safe_read_region(top_left, vis_level, region_size).convert("RGB"))
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=224, step_size=224, save_coord=True, **kwargs):
    ...
    def createPatches_bag_pngs(self, save_path, patch_level=0, patch_size=224, step_size=224, save_coord=True, **kwargs):
        contours = self.contours_tissue
        contour_holes = self.holes_tissue
    ...
    #         for patch in patch_gen:
    #             parent_dir = os.path.join(save_path, patch.get('name'))
    #             os.makedirs(parent_dir, exist_ok=True)

    #             patch_PIL = patch.get('patch_PIL')
    #             patch_name = f"{patch.get('x')}_{patch.get('y')}.png"
    #             patch_PIL.save(os.path.join(parent_dir, patch_name))

    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path,
        patch_size=224, step_size=224, custom_downsample=1, rescale_factor=None,
        white_black=True, white_thresh=15, black_thresh=50, contour_fn='four_pt', use_padding=True):

        output_patch_size = patch_size
        use_rescale = rescale_factor is not None

        if use_rescale:
            rf = rescale_factor
            read_patch_size = round(patch_size * rf)
            read_step_size = round(step_size * rf)
            needs_resize = (read_patch_size != output_patch_size)
            if needs_resize:
                print("Rescale: factor={:.4f}, reading {}x{} at level {}, output {}x{}".format(
                    rf, read_patch_size, read_patch_size, patch_level,
                    output_patch_size, output_patch_size))
        elif custom_downsample > 1:
            assert custom_downsample == 2
            rf = custom_downsample
            read_patch_size = patch_size * 2
            read_step_size = step_size * 2
            needs_resize = True
            print("Custom Downsample: {}, Patching at {} x {}, But Final Patch Size is {} x {}".format(
                custom_downsample, read_patch_size, read_patch_size,
                output_patch_size, output_patch_size))
        else:
            rf = 1
            read_patch_size = patch_size
            read_step_size = step_size
            needs_resize = False

        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        ref_patch_size = (read_patch_size * patch_downsample[0], read_patch_size * patch_downsample[1])
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                try:
                    patch_PIL = self.safe_read_region((x, y), patch_level, (read_patch_size, read_patch_size)).convert("RGB")
                except openslide.lowlevel.OpenSlideError as e:
                    print(f"Error reading region {(x, y, read_patch_size, read_patch_size)} from slide: {e}")
                    continue

                if needs_resize:
                    patch_PIL = patch_PIL.resize((output_patch_size, output_patch_size))
                ...
                if use_rescale:
                    patch_info = {
                        ...
                        'level0_dim': self.level_dim[0],
                        'output_patch_size': output_patch_size,
                        'read_patch_size': read_patch_size,
                        'read_step_size': read_step_size,
                        'level0_patch_size': ref_patch_size,
                        'level0_step_size': (step_size_x, step_size_y),
                        'coord_level': 0,
    ...
    @staticmethod
    def isInContours(cont_check_fn, pt, holes=None, patch_size=224):
        if cont_check_fn(pt):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=224, step_size=224, **kwargs):
        ...
        fp_chunk_size = math.ceil(n_contours * 0.05) if n_contours > 0 else 1
        init = True

        custom_downsample = kwargs.get('custom_downsample', 1)
        rescale_factor = kwargs.get('rescale_factor', None)

        for idx, cont in enumerate(self.contours_tissue):
            if (idx + 1) % fp_chunk_size == 0 or (idx + 1) == n_contours:
            ...
            if len(coords) > 0:
                ...
                attr_dict = { 'coords' : {'patch_size': patch_size, 'patch_level': patch_level,
                                          'downsample': self.level_downsamples[patch_level],
                ...
                if rescale_factor is not None:
                    pd = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
                    rps = round(patch_size * rescale_factor)
                    rss = round(step_size * rescale_factor)
                    attr_dict['coords'].update({
                        'output_patch_size': patch_size,
                        'read_patch_size': rps,
                        'level0_patch_size': (rps * pd[0], rps * pd[1]),
                        'level0_step_size': (rss * pd[0], rss * pd[1]),
                        'level0_dim': self.level_dim[0],
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    pt = (x_start, y_start)
                    canvas = np.array(self.safe_read_region(pt, vis_level, blend_block_size).convert("RGB"))
